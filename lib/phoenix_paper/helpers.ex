defmodule PhoenixPaper.Helpers do
  ...
  and merged with the caller-supplied `class` using `Tails` (so conflicting
  Tailwind utilities are deduplicated, last one wins).
  ...
  def classes(true, paper_classes, extra_class), do: Tails.classes([paper_classes, extra_class])
  def classes(false, _paper_classes, extra_class), do: Tails.classes([extra_class])
